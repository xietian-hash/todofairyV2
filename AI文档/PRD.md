...
## 变更补充（2026-09-05 子任务拖动排序）
1. 任务弹窗中的子任务新增拖动手柄，用户可拖动手柄调整子任务顺序；保存后按新顺序展示与生成。
2. 每条子任务保存内部稳定标识 `subTaskId`，该标识不对用户展示；拖动仅调整数组顺序，不改变标识。
3. 当天已生成的子待办按稳定标识匹配并更新展示顺序，已完成状态及其归属不因排序而改变；历史待办不回写。
4. 旧子任务数据在首次读取时按原顺序兼容，并在后续保存时补齐稳定标识。
5. 用户长按拖动手柄后，原列表保留半透明占位，并在弹窗最上层显示跟随手指移动的浮动子任务卡片，卡片顶部相对手指上移 20 像素，避免长列表时被内部滚动边界裁剪；激活时触发一次短震动，松手、取消或关闭弹窗后恢复正常样式。

## 变更补充（2026-09-05 子待办完整标题提示）
1. 仅子待办支持长按查看完整标题；父待办不展示该提示。
2. 长按子待办后，在其上方显示完整标题悬浮提示；顶部空间不足时改为显示在下方。
3. 手指移出当前子待办、松手、取消触摸或开始左滑时，提示立即隐藏；长按提示不触发待办编辑。
